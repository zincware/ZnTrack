...
def test_load_from_file_err():
    ...
    with pytest.raises(FileNotFoundError):
        example._load_from_file(
            file=pathlib.Path("params.yaml"), key="node_name", raise_error=True
        )

    assert (
        example._load_from_file(file=pathlib.Path("params.yaml"), key="node_name") is None
    )
